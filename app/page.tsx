...
export default function Home() {
  ...
  const handleSend = async (e) => {
    e.preventDefault();
    const newMessage = { id: crypto.randomUUID(), content: input, role: 'user' };
    setMessages([...messages, newMessage]);
    setInput('');

    try {
      const response = await fetch(
        "http://127.0.0.1:7860/api/v1/run/ba821b00-2338-4242-87c2-cdafb0edbaed?stream=false",
        {
          method: "POST",
          headers: {
            "Authorization": "Bearer <TOKEN>", // Replace with your actual token
            "Content-Type": "application/json",
            "x-api-key": "sk-d5uU4Z_uFWSyb0B1YWtu-FH91ZCJmOJtGNYVrwGgiGc" // Replace with your actual API key
          },
          body: JSON.stringify({
            input_value: input,
            output_type: "chat",
      ...
      const botMessage = { id: crypto.randomUUID(), content: data.outputs[0].outputs[0].results.message.text, role: 'bot' };
  ...
  const handlePrompt = async (promptText) => {
    const newMessage = { id: crypto.randomUUID(), content: promptText, role: 'user' };
    setMessages([...messages, newMessage]);
